...
float4 mainPS(VSO input) : COLOR0
{
	...
	float3 color = 0.0f;
	color+=manualSample(source, float2(input.uv.x - blurSize*4, input.uv.y), texSize)*0.05f;
	color+=manualSample(source, float2(input.uv.x - blurSize*3, input.uv.y), texSize)*0.09f;
	color+=manualSample(source, float2(input.uv.x - blurSize*2, input.uv.y), texSize)*0.12f;
	color+=manualSample(source, float2(input.uv.x - blurSize, input.uv.y), texSize)*0.15f;
