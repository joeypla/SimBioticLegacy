...
float4 mainPS(VSO input) : COLOR0
{
	...
	float4 color = 0.0f;
	color+=manualSample(source, float2(input.uv.x, input.uv.y - blurSize*5), texSize)*0.04f;
	color+=manualSample(source, float2(input.uv.x, input.uv.y - blurSize*4), texSize)*0.04f;
	color+=manualSample(source, float2(input.uv.x, input.uv.y - blurSize*3), texSize)*0.08f;
	color+=manualSample(source, float2(input.uv.x, input.uv.y - blurSize*2), texSize)*0.11f;
	color+=manualSample(source, float2(input.uv.x, input.uv.y - blurSize), texSize)*0.14f;
