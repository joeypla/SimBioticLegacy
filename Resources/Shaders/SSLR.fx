...
{
    float depth = tex2D(depthSampler, input.UV).x;
    float4 vpOrigin = 1.0f;
    vpOrigin.xy = input.UV;
    vpOrigin.z = tex2D(depthSampler, input.UV).x;
    vpOrigin = mul(vpOrigin, inverseViewProjection);
    vpOrigin /= vpOrigin.w;
    ...
    float3 tangent = normalize(normal * dot(randomVector, normal) - randomVector);
	...
	float3 samplePosition = kernelOffsets[i] * 0.1f + vpOrigin;
	
	float4 offset = float4(samplePosition, 1);
	offset = mul(offset, viewProjectionMatrix);
	offset /= offset.w;
	offset.z = offset.z;// * 0.5f + 0.5f;

	float sampleDepth = tex2D(depthSampler, offset.xy).x;
	
	float rangeCheck = 0.0f;//abs(LinearizeDepth(subjectDepth) - LinearizeDepth(sampleDepth));
    ...
    occlusion /= 32.0f;//numImportantSamples;
    occlusion = pow(occlusion, 3);
    //occlusion = clamp(occlusion - 0.3f + pow(occlusion, 5), 0.0f, 1.0f);
